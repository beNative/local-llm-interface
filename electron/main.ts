...
// FIX: Use a namespace import for Electron to resolve issues with named exports not being found.
...
electron.app.whenReady().then(() => {
    ...
    electron.ipcMain.handle('log:write', (_, entry: { timestamp: string, level: string, message: string }) => {
        try {
            const logDir = electron.app.isPackaged ? path.dirname(electron.app.getPath('exe')) : electron.app.getAppPath();
